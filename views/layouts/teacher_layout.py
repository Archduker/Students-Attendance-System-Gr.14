...
class TeacherLayout(ctk.CTkFrame):
    """
    Main layout for Teacher Pages.
    Includes:
    ...
    def set_active_page(self, page_key: str):
        ...
        print(f"DEBUG: Teacher Navigation -> {self.current_path}") 
    ...
    def _create_sidebar(self):
        ...
        self.menu_items = [
            ...
            ("Attendance Management", "session_management", "📅"),
            ("History", "history", "🕒"),
            ("Profile", "profile", "👤")
        ]
    ...
    def _create_main_area(self):
        ...
        # Dynamic User Data
        display_name = self.user.full_name if self.user else "Teacher"
        # Since 'role' might be an Enum or string, handle accordingly. 
        # Typically teacher objects might not have 'role' attribute exposed same way as students or might be different class.
        # Fallback to "Teacher" if not available.
        display_role = "Teacher"
        
        ctk.CTkLabel(info_frm, text=display_name, font=("Inter", 12, "bold"), text_color="black").pack(anchor="e")
        ctk.CTkLabel(info_frm, text=display_role, font=("Inter", 10), text_color="#10B981").pack(anchor="e") # Green for teacher
        
        # Avatar
        ctk.CTkLabel(user_area, text="PT", width=35, height=35, fg_color="#D1FAE5", text_color="#059669", font=("Inter", 12, "bold"), corner_radius=17).pack(side="left", padx=(10, 0))
